...
\begin{denselist}
  \item {\bf Store Orders dataset}~\cite{superstore}: This dataset is
    ...
    %enable us to demonstrate how \VizRecDB can correctly deal with
...
% \stitle{Demonstrating Utility:} To show the utility of \VizRecDB\ in a real-world
...
% selected. To aid the evaluation of visualizations, the demo version of \VizRecDB\
...
\eat{
We propose to demonstrate the functionality of the VizRecDB system by means of
analyzing three diverse datasets of practical importance. Users will be able to
explore each of these datasets in real-time by using VizRecDB to formulate
...
\begin{itemize}
  ...
  richness of various data types. It will show off VizRecDB capabilities to
...
using the VizRecDB query builder or by using ready-made queries (e.g. selecting
outliers in a column etc.) and submits the query to VizRecDB. VizRecDB then searches
...
considers most interesting. The VizRecDB frontend then visualizes the top-{it k} views and
