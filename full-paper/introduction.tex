...
Consider the (fictional) Android app analytics team at Google. 
This team is tasked with studying the behavior of various apps on the Android
...
\noindent 
\begin{align*}
& \tt Q \ \ = \ \ SELECT \ * \ FROM \ \  AppMetrics \ \ WHERE \\
& \tt AppName=``App-A''
\end{align*}
...
constructing diverse views or visualizations from the data.
...
times by carrier.
To construct these views, the analyst can use operations such as binning,
grouping, and aggregation, and then generate visualizations from the view.
For example, to generate the view  for average load times by carrier, the
...
& \tt WHERE\ AppName=``App-A'' \ \ GROUP  \ \ BY \ \ carrier
\end{align*}
...
\ffigbox{%
  ...
  \caption{Scenario B: Average Load Times by Carrier}\label{fig:staplerX-b}%
}
\end{floatrow}
\vspace{-12pt}
\end{figure}
...
For instance, the view of load times by carrier, as shown in Figure
\ref{fig:staplerX}, may be interesting if the overall load times for all
products showed the {\it opposite} trend (e.g. Figure \ref{fig:staplerX-a}).
However, the same view may be uninteresting if the load times of all apps
...
identifies and highlights to the analyst the most interesting views of the query results using methods based on
deviation}. 
Specifically, \VizRecDB\ explores the space of all possible views and
...
\squishlist
  ...
  underlying data}.
  This is the Android metrics use case discussed above.
  \item {\it Use Case II: Comparison of a subset of data against all the
  remaining data}.
  Imagine that a medical ressearcher is studying a medical records dataset and
  wants to compare the set of patients with high medical expenses vs. the
  all the remaining patients. The techniques from the first use case can be used
  directly for this use case too.
  \item {\it Use Case III: Comparison of two distinct datasets}. Finally,
  imagine that in the Android metrics example, an analyst would like to compare
...
\cite{DBLP:conf/vldb/Parameswaran2013} described the vision for \VizRecDB, and
\cite{XXX} demonstrated a prototype of the \VizRecDB\ system.
In this work, we explored two distinct implementations of \VizRecDB, both on top
of a database.
First, we explore \VizRecDB\ as a wrapper on top of a database system and study
how far we could push existing systems to support a \VizRecDB-type workload.
Next, we overcome the constraints of the DBMS API by writing a custom system
that extensively shares results between views and performs aggressive pruning
...
\squishlist
  \item In keeping with previously published \VizRecDB\ vision, we
  ...
  finding the best visualizations as a variant of both top-k ranking and the
