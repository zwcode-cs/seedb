...
the user must manually specify a large number of visualizations, explore behavior of various
attributes (and combinations), and examine different subsets of data before finally 
arriving at visualizations that shows something valuable.
...
``useful'' or {\em high-utility} visualizations to support fast visual analysis.
...
\stitle{Illustrative Example.}
Consider a smartphone app analytics team that is tasked with answering why a certain app, BadApp, is showing poor performance and receiving consumer complaints. 
Suppose that the team uses the AppMetrics database containing metrics such as network usage, 
power consumption, load times etc. to perform the analysis.
The analyst begins by querying the AppMetrics database for BadApp metrics and uses her 
...
comparison of mobile operating systems running BadApp vs. all apps --- the list goes on.
Depending on the types of visualizations created, the number of possible visualizations grows exponentially with the number of metrics in the database.
...
\mpv{remove table and update viz?}
...
helpful if it would be based on a real dataset actually used in the evaluations. Using the illustrative example was helpful and quite motivating.
...
\mpv{sure, if we have time}

What might make one of these visualizations ``interesting'' or valuable for the 
task at hand depends on a number of factors.
Since our goal with \SeeDB is to make visual {\em analytics} faster, we adopt a metric
that prefers visualizations showing interesting or surpising trends in data.
...
In other words, we posit that {\em a visualization is potentially ``interesting'' if it shows
...
outline in Section \ref{sec:discussion} some means by which \SeeDB may be extended to support them.
...
into make visualization recommendations. \mpv{does this sound ok?}
