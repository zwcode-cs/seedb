...
\resolved{\mpv{Depnding on discussion flow: As noted previously, in addition to our deviation-based metric,
...
%  wrapper around a database and another a custom solution (Section~\ref{sec:system_architecture}).
  ...
  (Section~\ref{sec:experiments}) \mpv{Re-word based on Aditya's S3}. 
...
\mpv{Reword based on S3} Specifically, this work builds upon the \SeeDB vision by proposing a novel, general-purpose view 
