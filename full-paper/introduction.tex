...
She uses the Census data~\cite{census}
...
The example above indicates that visualizations that depict
...
Depending on discussion flow: As noted previously, in addition to our deviation-based metric,
...
\mpv{This doesn't quite fit here}.
We design \SeeDB as an end-to-end visualization platform where analysts can both generate 
their own visualizations (like Tableau or Spotfire), or get data-driven recommendations on demand.
We chose to support both modes of visualizations (automated and manual) in \SeeDB because we 
...
%  wrapper around a database and another a custom solution (Section~\ref{sec:system_architecture}).
  ...
  (Section~\ref{sec:in_memory_execution_engine}).
