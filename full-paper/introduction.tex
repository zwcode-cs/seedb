...
However, with high-dimensional datasets, finding visualizations that 
show interesting variations or patterns in the data is a non-trivial task:  a
...
In this paper, we tackle the problem of automatically finding such valuable visualizations.
...
\item past history of interactions of this user or other users with the dataset ({\it user history}), and
\item visual qualities of visualizations ({\it aesthetics}). 
\end{asparaenum}

Current recommendations systems provided by systems like Spotfire~\cite{} and Tableau~\cite{} leverage basic attribute types to suggest
visualizations that follow standard visualization best-practices from a user-supplied query, thus focusing on only the metadata, query,
and aesthetics  of this taxonomy.  No existing system that we are aware of 
...
To this end, in this work, we develop a system, called {\it \SeeDB}, that focuses less on the aesthetics of good visualizations, and more
...
Even in the area of data-driven recommendations, one can imagine a large variety of ways to model the quality or utility
of a visualization.
...
In this paper, we develop a general suite of techniques for efficiently searching the space of such visualizations.
Our techniques work with a wide range of {\it utility functions} that capture the goodness of a visualization according
to some metric (e.g., skewness of outliers in a data set).  We then evaluate our approach on a particular utility function
that that uses
deviation as means to quantify utility of a visualization and show via user studies that this often
highlight useful or interesting trends in data.
We note that of the vision for \SeeDB\ has been described in a previous vision paper~\cite{DBLP:conf/vldb/Parameswaran2013}, but
...
interesting visualizations, or {\it views} at interactive time scales.
Specifically, we describe two approaches to searching the space of possible views.
...
In our second approach, we implement a system that  shares sequential scans and query results between views, and 
