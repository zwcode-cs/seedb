...
serially executes individual aggregate 
view queries, two for each possible view.
...
We find that our sharing optimizations can provide up to a 40X speedup for ROW and 10X speedup for COL allowing small and moderate sized datasets to be processed at interactive time scales ($<$5s) and reduce latency on large datasets from 
...
{\em \underline{Summary:} Combining aggregate 
...
We see a similar trend for COL, but with a space budget of 100.\footnote{\scriptsize The different space
...
\agp{Check: In both cases, bin packing offers a significant performance
