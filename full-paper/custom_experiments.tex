...
\reviewer {
	If delta K is low the accuracy of the views are not sufficiently high
specially for the top 5 views it seems that the system cannot suggest the
views accurately. The authors have mentioned that the latency can be
lowered dramatically using pruning for the smaller K but at the same time
pruning will reduce the accuracy for smaller K's dramatically (accuracy of less
than 60\%)
}
