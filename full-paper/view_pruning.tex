...
The previous sections described the \SeeDB\ execution engine;
next, we discuss the view generator which is invoked before the
execution engine and is responsible for using table metadata
to prune redundant views.
To illustrate the concept of redundant views, consider a hypothetical dataset that stores the names of airports along
with the corresponding airport code. 
Since airport names and airport codes have a 1:1 relationship, views produced by grouping data along each of these attributes will be identical.
Consequently, it would suffice to compute and recommend only one of these views while providing the user the ability to explore the redundant view if required.
...
\mpv{should we have figures of real views?}
...
The view generator largely works offline, and adopts the following steps to prune redundant views: 
(1) For each table, the view generator uses metadata about
attributes to determine the entire space of views. 
(2) The view generator prunes all views containing attributes with 0 or low variance since
...
View pruning allows us to significantly reduce the number of views (and execution time) in real
