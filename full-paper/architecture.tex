...
{\em mixed initiative} \agp{cite?} data exploration tool.

\agp{delete rest, too detailed:
...
\SeeDB server is composed of \agp{XXX} main components.  The first
...
will be sent to the \SeeDB client 
and are displayed as visualization recommendations to the user, 
who can then interact with these
visualizations. \agp{I wasn't sure what view processor does... }

\stitle{Execution Engine.} In evaluating the collection of aggregate views, 
...
if divided into $n$ parts. (We will describe how to select $n$ later on.) \agp{Manasi -- if we do this, we need to add a fwd pointer.}
...
specifically, the sharing based optimizations in Section~\ref{sec:sharing_opt} and the pruning based optimizations in Section~\ref{sec:in_memory_execution_engine}.
