...
\srm{what dataset are we using for this evaluation?  how many columns?  what
data distribution?}
...
columns to be read.  \srm{It's hard to read the column store plot.  Consider log Y scale.  Also report the maximum 
speedup for both columns and rows.}  \srm{Do we think these results will generalize to other data sets?  Are they sensitive to
...
was independently generated and had exactly 10 distinct values. \srm{Is this different than what was done above?  If we use the
same distribution / generation function for all of these queries we should just say that above.  If we use different data sets for
every experiment we should explain why we think that's an OK thing to do.}  
...
with $p$ group-by attributes is $10^p$.  \srm{It can't really be $10^P$ if we scale $P$ up to 20!  Maybe its max($10^P$,num rows)?}
...
groups=$10^4$.
Beyond $10^5$, the performance degrades drastically.   \srm{same comment as above -- we can't really have $10^{20}$ groups!}
...
number of distinct values in each dimension attribute.   \srm{How is 2b different from 2c?  Why not just show dotted line on 2c?}
...
upper limit.
\srm{Need to fix graphs as we discussed.}
...
optimization.  \srm{need to explain why}
...
We explictly choose not to directly compare the latency
