...
(b) the individual optimizations detailed in Section~\ref{sec:optimizations},
...
allowing near-interactive response times. This is {\em especially impressive given that, implicitly,
\VizRecDB is running 100s of queries on the DBMS for returning responses to a single \VizRecDB query}.

\item Overall, column stores are vastly superior to row stores 
...
of views is increased. Column stores are vastly superior to row stores,
often with 1/5th the latency.}
...
For these experiments, we used the SYN1 dataset and created subsets of SYN  with
...
\begin{figure*}[t]
	...
	\begin{subfigure}{0.33\linewidth}
		...
		\caption{Latency vs. number of aggregates \srm{Delete the n =2 bar to make it look more normal.}}
		\label{fig:multi_agg}
	\end{subfigure}
