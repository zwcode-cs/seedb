...
\purple{Our current system uses a particular metric for defining visualization utility,
namely, deviation-based utility.
Clearly there are other metrics and dimensions that can capture the interestingness
of a visualization.
...
\purple{In addition to data, many other aspects determine the interestingess of a visualization.
...
making visual design a important metric in visualization recommendation.
Finally, user preference, whether based on history of multiple users or a single user in
a specific session, can provide insight into which visualizations are most interesting.
...
In our technical report, we discuss how we can design a generalized utility metric to 
