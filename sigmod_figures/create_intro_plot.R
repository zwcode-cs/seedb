...
ggplot(data=demo1) + theme_bw() + geom_bar(aes(x=month, y=avg_profit_StaplerX), stat="identity") + 
	ylab("Avg. load time (ms") + xlab("Carrier") + theme(text=element_text(size=24))
ggsave(scale=1.5, file="../full-paper/Images/dist1.pdf")
dev.off()
...
ggplot(data=demo1) + theme_bw() + geom_bar(aes(x=month, y=avg_profit_StaplerX), stat="identity") + 
	ylab("Avg. load time (ms") + xlab("Carrier") + theme(text=element_text(size=24))
ggsave(scale=1.5, file="../full-paper/Images/dist2.pdf")
dev.off()
...
ggplot(data=demo1) + theme_bw() + geom_bar(aes(x=month, y=avg_profit_StaplerX), stat="identity") + 
	ylab("Avg. load time (ms") + xlab("Carrier") + theme(text=element_text(size=24))
ggsave(scale=1.5, file="../full-paper/Images/dist3.pdf")
dev.off()
