old <- function() {
	tmp = read.table("/Users/manasi/Documents/workspace/seedb/sigmod_figures/multi_gb.txt", sep="\t")
	colnames(tmp) = c("dataset", "size", "dims", "measures", "n_GB", "latency", "dbms")
	tmp$dbms = factor(tmp$dbms)
	tmp$n_GB = factor(tmp$n_GB)
	ggplot(tmp, aes(n_GB, latency/1000)) + geom_bar(aes(fill=dbms, ordered=TRUE), position="dodge", stat="identity") + ylab("latency (s)")
	ggsave(file="/Users/manasi/Documents/workspace/seedb/sigmod_figures/in_memory_latency.pdf")
}
...
multi_gb <- function(filename) {
	...
	cols = gg_color_hue(2)
        ...
        ggplot(tmp, aes(groups, latency/1000, color=dbms)) + geom_line() + geom_point() + theme(text = element_text(size=24)) + ylab("latency(s)") + scale_x_log10(name="Number of groups");
